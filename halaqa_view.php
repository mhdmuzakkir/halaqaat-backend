...
function gender_label($tr, $g){ return normalize_gender($g)==='girl' ? $tr['girl'] : $tr['boy']; }
function session_label($tr, $s){ return normalize_session($s)==='asr' ? $tr['shaam'] : $tr['subah']; }
...
if ($f_gender !== '') { $where[]="h.gender=?"; $params[]=$f_gender; $types.='s'; }
if ($f_session !== '') { $where[]="h.session=?"; $params[]=$f_session; $types.='s'; }
if ($f_state !== '') {
  if ($halaqaatHasState) { $where[]="h.state=?"; $params[]=$f_state; $types.='s'; }
  else {
...
<head>
  ...
  <style>
    html[lang="en"] body, html[lang="en"] * { font-family:'Montserrat', system-ui, -apple-system, Segoe UI, Arial, sans-serif !important; }
    html[lang="ur"] body, html[lang="ur"] * { font-family:'Noto Nastaliq Urdu', serif !important; }
    ...
    *{box-sizing:border-box}
    ...
    .sideToggle{border:1px solid rgba(255,255,255,.25);background:rgba(255,255,255,.08);color:#fff;padding:8px 10px;border-radius:12px;font-weight:900;cursor:pointer;}
    ...
    .nav a.dash::before{-webkit-mask-image:url('data:image/svg+xml;utf8,<svg xmlns="http://www.w3.org/2000/svg" fill="black" viewBox="0 0 24 24"><path d="M3 13h8V3H3v10zm0 8h8v-6H3v6zm10 0h8V11h-8v10zm0-18v6h8V3h-8z"/></svg>');}
    .nav a.halaqa::before{-webkit-mask-image:url('data:image/svg+xml;utf8,<svg xmlns="http://www.w3.org/2000/svg" fill="black" viewBox="0 0 24 24"><path d="M4 6h16v2H4zm0 5h16v2H4zm0 5h16v2H4z"/></svg>');}
    .nav a.students::before{-webkit-mask-image:url('data:image/svg+xml;utf8,<svg xmlns="http://www.w3.org/2000/svg" fill="black" viewBox="0 0 24 24"><path d="M16 11c1.66 0 2.99-1.34 2.99-3S17.66 5 16 5s-3 1.34-3 3 1.34 3 3 3zM8 11c1.66 0 2.99-1.34 2.99-3S9.66 5 8 5 5 6.34 5 8s1.34 3 3 3zm0 2c-2.33 0-7 1.17-7 3.5V19h14v-2.5C15 14.17 10.33 13 8 13zm8 0c-.29 0-.62.02-.97.05 1.16.84 1.97 1.97 1.97 3.45V19h6v-2.5c0-2.33-4.67-3.5-7-3.5z"/></svg>');}
    .nav a.ustaaz::before{-webkit-mask-image:url('data:image/svg+xml;utf8,<svg xmlns="http://www.w3.org/2000/svg" fill="black" viewBox="0 0 24 24"><path d="M12 3L1 9l11 6 9-4.91V17h2V9L12 3z"/></svg>');}
    .nav a.exams::before{-webkit-mask-image:url('data:image/svg+xml;utf8,<svg xmlns="http://www.w3.org/2000/svg" fill="black" viewBox="0 0 24 24"><path d="M19 3h-4.18C14.4 1.84 13.3 1 12 1s-2.4.84-2.82 2H5c-1.1 0-2 .9-2 2v16c0 1.1.9 2 2 2h14c1.1 0 2-.9 2-2V5c0-1.1-.9-2-2-2z"/></svg>');}
    .nav a.reports::before{-webkit-mask-image:url('data:image/svg+xml;utf8,<svg xmlns="http://www.w3.org/2000/svg" fill="black" viewBox="0 0 24 24"><path d="M3 13h2v-2H3v2zm0 4h2v-2H3v2zm4 0h14v-2H7v2zm0-4h14v-2H7v2z"/></svg>');}
    .nav a.settings::before{-webkit-mask-image:url('data:image/svg+xml;utf8,<svg xmlns="http://www.w3.org/2000/svg" fill="black" viewBox="0 0 24 24"><path d="M19.14 12.94c.04-.31.06-.63.06-.94s-.02-.63-.06-.94l2.03-1.58c.18-.14.23-.41.12-.61l-1.92-3.32c-.11-.2-.36-.28-.57-.22l-2.39.96c-.5-.38-1.04-.7-1.64-.94L14.5 2h-5l-.37 2.35c-.6.24-1.14.56-1.64.94l-2.39-.96c-.21-.06-.46.02-.57.22L2.61 7.87c-.11.2-.06.47.12.61l2.03 1.58c-.04.31-.06.63-.06.94s.02.63.06.94L2.73 14.52c-.18.14-.23.41-.12.61l1.92 3.32c.11.2.36.28.57.22l2.39-.96c.5.38 1.04.7 1.64.94L9.5 22h5l.37-2.35c.6-.24 1.14-.56 1.64-.94l2.39.96c.21.06.46-.02.57-.22l1.92-3.32c.11-.2.06-.47-.12-.61l-2.03-1.58z"/></svg>');}
    ...
    .pillSide{display:block;text-align:center;text-decoration:none;color:#fff;border:1px solid rgba(255,255,255,.25);background:rgba(255,255,255,.08);padding:10px 12px;border-radius:12px;font-weight:900;}
    ...
    .btnAdd{
      ...
      background:var(--primary);color:#fff;font-weight:900;white-space:nowrap;box-shadow:0 6px 18px rgba(0,0,0,.06);
    }
    .btnAdd:hover{filter:brightness(.96);}

    /* Combined language toggle pill: "ار | EN" */
    .langPill{
      display:inline-flex;align-items:center;overflow:hidden;border-radius:12px;border:1px solid var(--border);
      background:#fff;box-shadow:0 6px 18px rgba(0,0,0,.04);
    }
    .langPill a{
      text-decoration:none;padding:10px 12px;font-weight:900;font-size:13px;display:inline-block;color:var(--accent);
      min-width:56px;text-align:center;
    }
    .langPill .sep{padding:0 4px;color:#999;font-weight:900;}
    .langPill a.activeEn{background:var(--primary);color:#fff;}
    .langPill a.activeUr{background:var(--secondary);color:#fff;}

    .statsMini{display:flex;gap:10px;align-items:center;flex-wrap:wrap;}
    .badgeMini{
      display:inline-block;padding:10px 12px;border-radius:12px;border:1px solid var(--border);font-weight:900;font-size:13px;
      background:#fff;white-space:nowrap;
      box-shadow:0 6px 18px rgba(0,0,0,.04);
    }
    .badgeMini.halqaat{background:rgba(62,132,106,.14);border-color:rgba(62,132,106,.40);color:var(--primary);}
    .badgeMini.students{background:rgba(177,143,110,.16);border-color:rgba(177,143,110,.42);color:var(--secondary);}
    ...
    .searchWrap input{width:100%;border:0;outline:none;background:transparent;font-size:14px;color:var(--accent);}

    /* Filter bar: PRIMARY background */
    .filtersRow{
      margin-bottom:14px;border-radius:16px;padding:12px;
      background:var(--primary);
      display:flex;align-items:center;justify-content:space-between;gap:10px;flex-wrap:wrap;
      box-shadow:0 8px 20px rgba(0,0,0,.06);
    }
    .chip{display:inline-flex;align-items:center;gap:8px;padding:8px 10px;border-radius:12px;background:rgba(255,255,255,.14);border:1px solid rgba(255,255,255,.25);font-weight:900;font-size:13px;color:#fff;white-space:nowrap;}
    .filtersRight{display:flex;gap:10px;flex-wrap:wrap;align-items:center;}

    select{
      appearance:none;-webkit-appearance:none;-moz-appearance:none;
      padding:10px 36px 10px 12px;border:1px solid rgba(255,255,255,.35);border-radius:12px;background:#fff;
      font-size:13px;font-weight:900;outline:none;
      background-repeat:no-repeat;background-size:14px 14px;background-position: calc(100% - 12px) 50%;
    ...
    html[dir="rtl"] select{background-position: 12px 50%;padding:10px 12px 10px 36px;text-align:right;}
    ...
    /* Add panel */
    .addPanel{background:#fff;border:1px solid var(--border);border-radius:16px;box-shadow:0 6px 18px rgba(0,0,0,.04);overflow:hidden;margin-bottom:14px;display:none;}
    ...
    input[type="text"]{width:100%;padding:10px 12px;border:1px solid var(--border);border-radius:12px;outline:none;background:#fff;font-size:14px;}
    input:focus, select:focus{border-color:rgba(62,132,106,.55); box-shadow:0 0 0 4px rgba(62,132,106,.12);}
    .row2{display:grid;grid-template-columns:1fr 1fr;gap:10px;}
    @media(max-width:520px){.row2{grid-template-columns:1fr;}}
    .btnSave{width:100%;margin-top:14px;padding:12px;border:0;border-radius:12px;background:var(--primary);color:#fff;font-weight:900;cursor:pointer;font-size:15px;}
    ...
    .card{
      ...
      box-shadow:0 6px 18px rgba(0,0,0,.04);overflow:hidden;display:flex;flex-direction:column;min-height:168px;
    }
    .cardTop{padding:14px 14px 10px;display:flex;align-items:flex-start;justify-content:space-between;gap:10px;}
    ...
    .goBtn{border:0;background:transparent;cursor:pointer;font-size:20px;line-height:1;color:#777;margin-top:4px;}
    .cardMid{padding:0 14px 12px;display:flex;flex-wrap:wrap;gap:8px;align-items:center;}

    /* Tags now small rounded rectangles (like pic) */
    .tag{
      display:inline-block;
      padding:6px 10px;
      border-radius:10px;        /* rectangle-ish */
    ...
    .tag.boy{background:rgba(47,111,214,.16);border-color:rgba(47,111,214,.40);color:#1b3f86;}
    .tag.girl{background:rgba(210,78,138,.16);border-color:rgba(210,78,138,.40);color:#7a1f4a;}
    .tag.subah{background:rgba(255,140,0,.18);border-color:rgba(255,140,0,.45);color:#6a3a00;}
    .tag.shaam{background:rgba(120,70,20,.18);border-color:rgba(120,70,20,.45);color:#4a2a0d;}
    .tag.active{background:rgba(62,132,106,.14);border-color:rgba(62,132,106,.40);color:var(--primary);}
    .tag.paused{background:rgba(255,165,0,.16);border-color:rgba(255,165,0,.45);color:var(--paused);}
    .tag.stopped{background:rgba(177,28,28,.14);border-color:rgba(177,28,28,.40);color:var(--stopped);}

    .cardFoot{margin-top:auto;padding:12px 14px;border-top:1px solid var(--border);display:flex;justify-content:space-between;gap:10px;font-size:12px;color:#666;align-items:center;}
    ...
    html[dir="rtl"] .resultsLine{justify-content:flex-start;}

    /* Mobile sidebar */
    .menuBtn{
      display:none;background:#3e846a;border:1px solid rgba(255,255,255,.6);color:#fff;
