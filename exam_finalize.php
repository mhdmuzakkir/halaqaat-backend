...
error_reporting(E_ALL);               // set to 0 in production
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['finalize_exam'])) {
  if (!ob_get_level()) {
    ob_start();
  } // helps avoid "headers already sent"
...
try {
  ...
  $error = ($lang === 'ur' ? 'SQL مسئلہ: ' : 'SQL error: ') . $e->getMessage();
}
...
if ($selectedExam > 0) {
  ...
  $examTitleLower = strtolower($examDetails['title'] ?? '');
  $examType = 'qaida';
  if (strpos($examTitleLower, 'حفظ') !== false || strpos($examTitleLower, 'hifz') !== false) {
    $examType = 'hifz';
  } elseif (strpos($examTitleLower, 'ناظرہ') !== false || strpos($examTitleLower, 'nazira') !== false) {
    $examType = 'nazira';
  }

  // Get all draft results for this exam
  try {
    $stmt = $conn->prepare("
      SELECT er.*,
             s.full_name_ur,
...
?>
  ...
  $avgPercentage = $totalStudents > 0 ? (array_sum(array_map(fn($r) => (float)$r['percentage'], $results)) / $totalStudents) : 0;
            ...
            ?>
                <tr style="background: rgba(170, 129, 94, 0.15);">
                  <td colspan="16" class="fw-bold" style="padding: 8px 16px;">
                    <i class="bi bi-building me-2"></i><?php echo h($currentHalaqa); ?> (<?php echo h($tr[$r['halaqa_gender']] ?? ($r['halaqa_gender'] ?? '')); ?>)
                  </td>
                </tr>
              <?php endif; ?>
              ...
              <tr>
                ...
                <td>
                  ...
                  <?php if (!empty($r['student_mumayyaz'])): ?>
                    <span style="background: rgba(15,45,61,0.15); color: #0f2d3d; padding: 2px 6px; border-radius: 4px; font-size: 10px;">
                      <i class="bi bi-star-fill"></i>
                    </span>
                  <?php endif; ?>
                </td>
                <td><?php echo h($r['halaqa_name'] ?? ''); ?></td>
                <td><?php echo h($tr[$r['shuba']] ?? ($r['shuba'] ?? '')); ?></td>
                <td class="text-center"><?php echo h($r['nisaab_from'] ?: '-'); ?></td>
                <td class="text-center"><?php echo h($r['nisaab_to'] ?: '-'); ?></td>
                <td class="text-center"><?php echo h($r['nisaab_marks'] ?? '0'); ?></td>
                <?php if ($examType !== 'qaida'): ?>
                  <td class="text-center"><?php echo h($r['husn_sawt'] ?? '0'); ?></td>
                <?php endif; ?>
                <td class="text-center"><?php echo h($r['tajweed'] ?? '0'); ?></td>
                <td class="text-center"><?php echo h($r['izaafat'] ?? '0'); ?></td>
                <td class="text-center"><?php echo h($r['sulook'] ?? '0'); ?></td>
                <td class="text-center fw-bold"><?php echo h($r['marks_obtained'] ?? '0'); ?></td>
                ...
                <td class="text-center fw-bold"><?php echo h($r['taqdeer'] ?? ''); ?></td>
                <td class="text-center">
                  <?php if (!empty($r['mumayyaz'])): ?>
                    <span style="background: rgba(15,45,61,0.15); color: #0f2d3d; padding: 4px 8px; border-radius: 12px; font-size: 11px;">
                      <i class="bi bi-star-fill"></i> <?php echo h($tr['mumayyaz']); ?>
                    </span>
                  <?php else: ?>
