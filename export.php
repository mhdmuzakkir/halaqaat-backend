...
switch ($type) {
    case 'halaqa':
        ...
        fputcsv($output, ['Name', $halaqa['name']]);
        fputcsv($output, ['Teacher', $halaqa['teacher_name']]);
        fputcsv($output, ['Gender', $halaqa['gender']]);
        fputcsv($output, ['Time', $halaqa['time_slot']]);
        fputcsv($output, []);
        
        // Students
        fputcsv($output, ['Students']);
        fputcsv($output, ['#', 'Name', 'Shuba', 'Roll Number', 'Mumayyiz', 'Avg Percentage']);
    ...
    case 'student':
        ...
        fputcsv($output, ['Father Name', $student['father_name']]);
        fputcsv($output, ['Mumayyiz', $student['is_mumayyiz'] ? 'Yes' : 'No']);
        fputcsv($output, []);
    ...
    case 'exam':
        ...
        $stmt = $conn->prepare("
            SELECT er.*, s.name as student_name, s.shuba, h.name as halaqa_name, h.gender
            FROM exam_results er
    ...
    case 'progress':
        ...
        fputcsv($output, ['Name', 'Shuba', 'Exams', 'Avg %', 'Attendance %', 'Mumayyiz']);
